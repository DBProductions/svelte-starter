...
test.describe('Table Component', () => {
  ...
  test('Sort table', async ({ page }) => {
    ...
    await expect(
      page.locator('table.tbl > tbody > tr:nth-child(1) > td:nth-child(2)')
    ).toHaveText('Svelte')
    await expect(
      page.locator('table.tbl > tbody > tr:nth-child(2) > td:nth-child(2)')
    ).toHaveText('Rollup')
    await expect(
      page.locator('table.tbl > tbody > tr:nth-child(3) > td:nth-child(2)')
    ).toHaveText('Cypress')
    await expect(
      page.locator('table.tbl > tbody > tr:nth-child(4) > td:nth-child(2)')
    ).toHaveText('Prettier')
